...
![](p8105_jl5934_hw6_files/figure-gfm/bootstrapping%20and%20isolating%20output%20of%20model-2.png)<!-- -->

- Our distribution of R2 values are primarily located at around 0.93,
  with a left sided tail nearing zero below 0.8

- Our distribution of log(B1\*B2) values are primarily located at 4.2,
  with a leftward tail extended below zero.
...
![](p8105_jl5934_hw6_files/figure-gfm/my%20birthweight%20model-1.png)<!-- -->

- Here we demonstrate a plot of the model’s predicted values for
  birthweight plotted against the residual values from my model. There
  is wide variability to the residuals, with most from -1000 to +1000,
  and most of the predicted values being \<3500 for birthweight.
...
![](p8105_jl5934_hw6_files/figure-gfm/main%20and%20alternate%20models%20for%20birthweights-2.png)<!-- -->

- Here we demonstrate two plots of the model’s predicted values for
  birthweight plotted against the residual values from the model.

  - The first is for the main model using main effects of birth length
    and gestational age. Compared to my model the residuals are a bit
    tighter but the spread is still from -1000 to +1000, although there
    is a greater range of predicted birthweights.

  - The second is the alternative including the interactions and main
    effects of head circumference, length at birth, and sex of the baby.
    Compared to the prior two models, the residual range is a bit
    tighter now primarily focused in -500 to +500, with still a wide
    range of predicted values similiar to the main model.
