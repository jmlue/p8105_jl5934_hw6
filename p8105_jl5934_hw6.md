...
```
    ...
    ## Warning: Removed 3676 rows containing non-finite values (`stat_density()`).
...
- Here we demonstrate a plot of the model’s predicted values for
  birthweight plotted against the residual values from my model.
...
- Here we demonstrate two plots of the model’s predicted values for
  birthweight plotted against the residual values from the model. The
  first is for the main model using main effects of birth length and
  gestatational age. The second is the alternative including the
  interactions and main effects of head circumference, length at birth,
  and sex of the baby.
